...
func Profile(w http.ResponseWriter, r *http.Request) {
	claims, ok := r.Context().Value(utils.ContextUserKey).(*utils.JwtClaims)
	if !ok || claims == nil {
	...
	utils.SendJson(w, http.StatusOK, model.Response{
		...
		Data:    profileFromClaims(claims),
	})
}

func profileFromClaims(claims *utils.JwtClaims) model.PublicUser {
	for _, user := range users {
		if user.Id == claims.UserID {
			return model.ToPublicUser(user)
		}
	}
